    ...
    , IReadOnlyList<T>
{
    ...
    ArrayBuilder(ObjectPool<ArrayBuilder<T>> pool)
        : this()
    {
        _pool = pool;
    }
    ...
    /// </summary>
    /// <typeparam name="U">The sub-class to cast to.</typeparam>
    ...
    public ImmutableArray<T> ToImmutableAndFree()
    {
        ...
        var result = Count switch
        {
            ...
            var n when n == _builder.Capacity => _builder.MoveToImmutable(),
            _ => ToImmutable(),
        };
    ...
    static readonly ObjectPool<ArrayBuilder<T>> PoolInstance = CreatePool();
    ...
    public static ObjectPool<ArrayBuilder<T>> CreatePool()
        => CreatePool(128); // we rarely need more than 10
